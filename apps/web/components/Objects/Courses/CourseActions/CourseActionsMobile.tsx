...
const CourseActionsMobile = ({ courseuuid, orgslug, course, trailData }: CourseActionsMobileProps) => {
  ...
  const getFirstAvailableActivity = () => {
    for (const chapter of course.chapters ?? []) {
      const activity = chapter.activities?.find((item) => !item.is_locked)
      if (activity) return activity
    }
    return null
  }

  const goToActivity = (activity: { activity_uuid: string }) => {
    router.push(
  ...
  const handleCourseAction = async () => {
    if (!session.data?.user) {
      const firstActivity = getFirstAvailableActivity()
      if (!isStarted && firstActivity) {
      ...
      <div className="flex flex-col space-y-4">
        ...
        {linkedOffers.length > 0 ? (() => {
          ...
          >
            {isActionLoading ? (
              <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
            ) : !session.data?.user && !getFirstAvailableActivity() ? (
              <>
